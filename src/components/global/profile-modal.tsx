...
export const ProfileModal = ({ user, isOpen, onRequestClose }: ProfileModalProps) => {
  ...
  return (
    <Modal
      ...
      className="mt-20 z-50 right-0 fixed font-romannew bg-dark-bg-secondary mr-10 border-2 border-border rounded-lg  flex flex-col " // Conteúdo do modal com fundo transparente
      style={{
        overlay: {
    ...
    >
      <div className="text-foreground text-4xl  w-fit flex flex-col p-3 rounded-lg gap-y-3  ">
        ...
        <Link to="/profile" className="flex gap-x-2">
          <div className="justify-center items-center hover:scale-110 duration-300 flex gap-x-2">
            <LuUser2 className="mt-1 font-extrabold text-2xl" />
            <span>Perfil</span>
          </div>
        </Link>
        <Link to="/login" className="flex gap-x-2" onClick={handleLogOut}>
          <div className="justify-center items-center hover:scale-110 duration-300 flex gap-x-3">
            <RiLogoutBoxLine className="mt-1 text-xl" />
            <span>Sair</span>
          </div>
        </Link>
      </div>
