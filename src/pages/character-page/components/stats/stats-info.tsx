...
export default function StatsInfo({ current_value, max_value, type }: StatsProps) {
  ...
  return (
    <div className="flex  items-center space-x-5 justify-between">
      ...
      <div className="text-4xl border border-primary font-light flex items-center pr-2  gap-x-2">
        <form onSubmit={handleSubmit(onSubmit)} className="flex  items-center  h-fit w-16  justify-center relative">
          <div className="flex justify-start  items-center -left-4">
            <button onClick={() => setPositive(false)} className="bg-[#FF4343] text-4xl text-black text-center font-extrabold flex items-center absolute justify-center rounded-full w-4 h-4">
          ...
          <div className="flex justify-end items-center">
            <button onClick={() => setPositive(true)} className="bg-[#75FF69] text-3xl absolute text-black text-center font-extralight flex items-center justify-center rounded-full w-4 h-4">
              +
            </button>
          </div>
        </form>
        {updatedCurrentValue} <span className="font-extrabold text-5xl ">/</span> {max_value}
      </div>
    </div>
  );
}
