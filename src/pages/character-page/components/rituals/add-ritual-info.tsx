    ...
    <div className={`flex flex-col border-[3px] border-border  `}>
      <div className="flex justify-between items-center cursor-pointer lg:p-6 md:p-4 p-2" onClick={() => setExpanded(!expanded)}>
        <div className="flex flex-col">
          <h1 className="lg:text-4xl md:text-3xl text-xl font-semibold">{ritual.name}</h1>
        </div>
        <button className="lg:text-5xl text-5xl font-bold">{expanded ? <IoMdArrowDropup /> : <IoMdArrowDropup className="rotate-180" />}</button>
      </div>

      <div
        className={`lg:text-4xl md:text-2xl text-xl  tracking-wide font-normal flex flex-col space-y-2 overflow-x-auto overflow-y-scroll lg:px-8 md:px-4 px-2 duration-300 transition-max-height  ${
      ...
      >
        <div className="font-extralight text-2xl ">
          <h3 className={`font-light text-2xl w-fit px-1 mb-2 ${elementColor.bg} ${elementColor.text_foreground} `}>
        ...
        {ritual.conditions.length > 0 && (
          <div>
            <h3 className="font-bold text-2xl">Condições:</h3>
            {ritual.conditions.map((condition) => (
              <p key={condition.condition.id} className="text-xl">
        ...
        <div className="flex items-center justify-end pt-5">
          <Button onClick={handleAddRitual} variant={'link'} className="text-primary ">
            Adicionar Ritual
          </Button>
        </div>
      </div>
